(function(root) {
	...
	test('encode', function() {
		...
		equal(
			...
			'Encodes disallowed code points in input'
		);
		raises(
			function() {
				he.encode(<%= invalidCodePointsString %>, { 'strict': true });
			},
			Error,
		...
		equal(
			...
			'&#x0;&#x89;',
			'Encodes disallowed code points in input when `encodeEverything: true`'
		);
	});
